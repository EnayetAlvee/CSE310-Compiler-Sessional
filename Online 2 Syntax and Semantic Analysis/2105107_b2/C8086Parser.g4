...
func_declaration returns[str_list var_list]: type_specifier ID LPAREN parameter_list RPAREN SEMICOLON
{
	...
	bool temp_bool = mySymbolTable.find($ID->getText());
...
func_definition returns[str_list var_list]: ts=type_specifier ID{
		...
		for (size_t i = 0; i < paramVars.size(); i++) {
			bool temp_bool=mySymbolTable.find(paramVars[i].second);
			cout<<"temp_bool: " << temp_bool <<" id: " << paramVars[i].second << endl;
			if(temp_bool){
				writeIntoErrorFile("Error at line " + std::to_string($ID->getLine()) + 
					": Multiple declaration of " + paramVars[i].second + "\n");
				writeIntoparserLogFile("Error at line " + std::to_string($ID->getLine()) +
...
var_declaration  returns [str_list var_list]
	...
	//   | t=type_specifier ID LTHIRD RTHIRD ASSIGNOP LCURL expression_list RCURL SEMICOLON {
    ...
    //         syntaxErrorCount++;
    //     }
    //     writeIntoparserLogFile(std::string("Line ") + std::to_string($SEMICOLON->getLine()) +
    //         std::string(": var_declaration: type_specifier ID LTHIRD RTHIRD ASSIGNOP LCURL expression_list RCURL SEMICOLON\n")
    //     );
	//   }
...
| BREAK{
	...
	if(do_flag==1){
		...
		writeIntoparserLogFile("Line "+$BREAK->getLine()+ " : break outside loop\n");
	}
...
|CONTINUE{
	...
	if(do_flag==1){
		...
		writeIntoparserLogFile("Line "+$CONTINUE->getLine()+" : CONTINUE outside loop\n");
	}
		...
		} WHILE LPAREN expression RPAREN SEMICOLON
	  {
		...
		std::to_string($SEMICOLON->getLine()) + 
		":statement: DO LCURL statements RCURL WHILE LPAREN expression RPAREN SEMICOLON\n");
		$var_list.add($DO->getText()+ $LCURL->getText()
		+$statement.var_list.get_list_as_string()+$RCURL->getText()
		+$WHILE->getText()+$LPAREN->getText()+$expression.var_list.get_list_as_string()+
		$RPAREN->getText()+$SEMICOLON->getText() +"\n"
		);
		...
		| PRINTLN LPAREN ID RPAREN SEMICOLON
	  {
		...
		writeIntoparserLogFile($PRINTLN->getText() + " " + $LPAREN->getText() + $ID->getText() + $RPAREN->getText() + $SEMICOLON->getText() + "\n");
	  }
...

	  {
		$var_list.setLineNumber($statement.var_list.getLineNumber());
		// $var_list.add($statement.var_list.get_list_as_string());
		writeIntoparserLogFile("Line " + std::to_string($statement.var_list.getLineNumber()) + ": statement : FOR LPAREN expression_statement expression_statement expression RPAREN statement\n");
		$var_list.add($FOR->getText() + " " + $LPAREN->getText() 
		+ $expression_statement.var_list.get_list_as_string() + $RPAREN->getText() 
	  ...
	  {
		$var_list.setLineNumber($statement.var_list.getLineNumber());
		writeIntoparserLogFile("Line " + std::to_string($statement.var_list.getLineNumber()) + ": statement : IF LPAREN expression RPAREN statement\n");
	  ...
	  | IF LPAREN expression RPAREN s1=statement ELSE s2=statement
	  {
		...
		+ $RPAREN->getText() + $s1.var_list.get_list_as_string() + "\n" + $ELSE->getText()+"\n"
		 + $s2.var_list.get_list_as_string());
		writeIntoparserLogFile($var_list.get_list_as_string() + "\n");
...
factor returns [str_list var_list]: variable 
{
	...
	| ID LPAREN RPAREN
		{
        $var_list.add($ID->getText() + $LPAREN->getText() + $RPAREN->getText());
        $var_list.setLineNumber($ID->getLine());
        writeIntoparserLogFile("Line " + std::to_string($ID->getLine()) + ": factor : ID LPAREN RPAREN\n");
        writeIntoparserLogFile($var_list.get_list_as_string() + "\n");
      }
	| ID LPAREN argument_list RPAREN
	{
...
// expression_list returns [str_list var_list]
//     : expression {
//         $var_list.add($expression.var_list.get_list_as_string());
//         $var_list.setLineNumber($expression.var_list.getLineNumber());
//         writeIntoparserLogFile("Line " + std::to_string($expression.var_list.getLineNumber()) + 
//             ": expression_list : expression\n");
//         writeIntoparserLogFile($var_list.get_list_as_string() + "\n");
//       }
//     | el=expression_list COMMA expression {
//         $var_list.set_variables($el.var_list.get_variables());
//         $var_list.add($COMMA->getText());
//         $var_list.add($expression.var_list.get_list_as_string());
//         $var_list.setLineNumber($expression.var_list.getLineNumber());
//         writeIntoparserLogFile("Line " + std::to_string($expression.var_list.getLineNumber()) + 
