...
public:
    ...
    string get_type_from_currentScope(const string& name) {
    ...
    bool find(string name){
        ...
        while (scope) {
            auto result = scope->lookup(name);
            if (result.first) {
                // out << "\tFound in ScopeTable# " << scope->getId() << " at position " << result.second.first << ", " << result.second.second << endl;
                return true;
            }
            scope = scope->getParentScope();
        }
        // out << "\tNot found in any ScopeTable" << endl;
        return false;
    }
